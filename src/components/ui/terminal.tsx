...
export const AnimatedSpan = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "block whitespace-pre-wrap break-words text-sm font-normal leading-relaxed tracking-tight",
        className
      )}
      onAnimationComplete={() => {
        if (!sequence) return
...
export const TypingAnimation = ({
  ...
  return (
    <MotionComponent
      ref={elementRef}
      className={cn(
        "block min-h-[1.5em] whitespace-pre-wrap break-words text-sm font-normal leading-relaxed tracking-tight",
        className
      )}
      {...props}
    >
...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "border-border bg-background z-0 w-full overflow-hidden rounded-xl border shadow-sm",
        className
      )}
    >
      <div className="border-border flex items-center gap-2 border-b px-4 py-3">
        ...
        <span className="ml-1 text-[11px] font-medium text-muted-foreground">kilobot — setup</span>
      </div>
      <pre className="m-0 overflow-x-hidden p-5 font-mono text-[13px] leading-6">
        <code className="grid gap-y-2 whitespace-pre-wrap break-words [overflow-wrap:anywhere]">
          {wrappedChildren}
        </code>
      </pre>
    </div>
