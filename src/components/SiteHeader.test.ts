...
test('the full header offer bar copies the Starter code', () => {
  ...
  expect(siteHeaderSource).toContain('px-4 py-1 text-center');
  ...
  expect(siteHeaderSource).toContain('gap-x-1.5 gap-y-0.5');
  expect(siteHeaderSource).toContain('from-[#eb0000] via-[#95008a] to-[#3300fc]');
  expect(siteHeaderSource).toContain('text-white');
  expect(siteHeaderSource).not.toContain('grid w-full place-items-center');
  expect(siteHeaderSource).not.toContain('from-amber-300');
  expect(siteHeaderSource).toContain('text-center');
  expect(siteHeaderSource).toContain('translate-y-[3px]');
  expect(siteHeaderSource).toContain('gap-1 font-bold tracking-wide');
