      ...
      <WebWidgetPreviewFrame device={previewDevice} onPointerDownCapture={() => undefined}>
        <div className={cn('relative flex h-full w-full items-end', mobilePreview ? 'mx-auto max-w-[390px]' : 'max-w-[460px] ml-auto')}>
          <div className={cn('absolute bottom-0 right-0 flex flex-col items-center gap-2', mobilePreview && 'gap-1.5')}>
            <a className={cn('flex max-w-[min(100%,320px)] items-center gap-2 rounded-full border-2 border-[#25D366] bg-white px-3.5 py-2 text-sm font-normal text-[#25D366]', mobilePreview && 'gap-1.5 px-3 py-1.5 text-[13px]')} style={{ fontFamily: 'Google Sans Flex, sans-serif' }} href={destinationUrl} target="_blank" rel="noreferrer">
