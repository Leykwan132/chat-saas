...
- Replace `You are on Business plan` with `Credits reset on Aug 28`.
...
- Preserve the current rows, progress bars, warning colors, and More credits action.
...
A focused source contract will assert that the card formats the reset date from `periodEndMs`, removes the redundant plan-status copy, preserves exact locale-formatted values, and uses the smaller typography classes.
