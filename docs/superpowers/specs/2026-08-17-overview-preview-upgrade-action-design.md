...
When the user selects Preview, render both sample analytics panels as today. Render one `Upgrade to unlock` outline button below the two-panel grid, aligned to the left edge of the preview area. Do not render an additional preview-level Upgrade button above the panels.
