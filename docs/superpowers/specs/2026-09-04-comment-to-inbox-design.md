...
The customer-facing navigation label is **Comment-to-Inbox**. A configured automation is called a **Comment automation**.

## Product behavior

The Comment-to-Inbox item appears in the dashboard sidebar directly above Tools. It opens a automation list with a create action, status toggle, sent count, and responded count for every automation.

A automation creation flow collects:
...
The automation is inactive until each selected page has a confirmed comment-event subscription. The user may activate or deactivate a automation after creation. An inactive automation never sends messages.

Free workspaces may store one automation total, whether active or inactive. Paid workspaces may store an unlimited number. The server enforces this rule; the client presents the appropriate upgrade state before creation.

## Automation pages and subscriptions

The page picker lists connected Instagram and Messenger channels available to the active workspace. A automation stores the selected channel IDs instead of copied access tokens or page credentials.

Activating a automation verifies and creates the required Meta comment subscriptions for each selected resource. Messenger page subscriptions are extended to include the relevant comment/feed event in addition to the existing messaging events. Instagram connections are extended to subscribe to the supported comment event for the selected Instagram business account.

If any selected resource cannot be subscribed, activation fails atomically and the automation stays inactive. The automation detail shows the exact failed page and Meta error, and the user can reconnect that page before retrying. Deactivation stops processing immediately; it does not remove the resource subscription, because another active automation may still rely on it.

## Data model

New tables are isolated from generic inbox records:

- `commentAutomations`: workspace ownership, name, status, trigger configuration, private message text, optional public reply text, and denormalized sent/responded counters.
- `commentAutomationPages`: one row per selected automation channel, including the current subscription status and most recent subscription error.
- `commentAutomationDeliveries`: one immutable row per automation/comment match, keyed by the source comment ID. It stores source channel, customer/conversation references, comment text and timestamp, private/public send outcomes, sent time, response time, and a failure reason where applicable.

Indexes cover workspace listing, automation pages by automation and channel, delivery lookup by source comment, automation detail pagination, and the unresolved delivery lookup required to mark a response. Counter fields are updated transactionally with delivery state changes rather than counted from a growing delivery table.
...
3. Locates matching active automations for that channel and evaluates the selected trigger.
...
The list provides automation name, active/inactive state, page count, trigger summary, sent, responded, and a creation action. A Free workspace at its limit sees an upgrade affordance rather than a second create flow.

The automation detail provides the current configuration, subscription health per selected page, sent/responded totals, activation controls, and a paginated delivery activity list. Each activity row shows the source comment, customer, private/public outcome, sent time, response state, and any failure reason.
...
The feature is intentionally unavailable to all other users until the allowlist is expanded or removed. Existing channel messaging and inbox behavior remain unchanged for comments that do not match an active automation.
...
- Free versus paid automation limit enforcement;
- automation creation, page selection, activation, and subscription failure behavior;
...
- authorization, page ownership, and inactive-automation guards.
