...
- Google Calendar HTTP calls go to WorkOS Relay with URL routing (`X-Relay-URL` = the Google Calendar URL, `X-Relay-User` = the WorkOS user). Do not send `X-Relay-Organization`. A 404 without `X-Relay-Upstream-Status` is a Relay failure, not a missing Google event.
...
- Google API calls run only in Convex actions or backend HTTP handlers. Each call is proxied through WorkOS Relay with `X-Relay-User` and no organization header.
