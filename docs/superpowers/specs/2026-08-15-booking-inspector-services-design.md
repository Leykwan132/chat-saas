...
- Each row shows the service name and its assigned-teammate count. In both the node and inspector, the count uses a dotted underline and pointer cursor to signal that hovering it lists the assigned teammates by name.
