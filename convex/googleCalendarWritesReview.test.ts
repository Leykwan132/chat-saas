...
function providerEvent(
  id: string, title: string, etag: string, operationKey?: string, payloadFingerprint?: string,
): GoogleCalendarEvent {
  return {
...
const dependencies = (t: CalendarTest, provider: typeof fetch) =>
...
test.each([undefined, "wrong"])(
  "sync imports normally when a create fingerprint marker is %s",
  async (providerFingerprint) => {
  const { t, connectionId, calendarEventId } = await setupEvent();
  const operationKey = `identity:fingerprint:${String(providerFingerprint)}`;
  let created: GoogleCalendarEvent | undefined;
  const write = dependencies(t, async (_request, init) => {
    const body = JSON.parse(String(init?.body)) as { id: string };
    created = providerEvent(
      body.id, eventInput.summary, '"created"', operationKey, providerFingerprint,
    );
    return Response.json(created);
  });
  write.finalizeEvent = async () => { throw new Error("interrupted"); };
  ...
  await runGoogleCalendarSync({
    connectionId, now: now + 1, dependencies: syncDependencies(t),
    listPage: async () => ({ items: [created!], nextSyncToken: "fingerprint_sync" }),
  });
  const operation = await t.run((ctx) => ctx.db.query("googleCalendarWriteOperations")
    .withIndex("by_operationKey", (q) => q.eq("operationKey", operationKey)).unique());
  expect(operation?.state).not.toBe("succeeded");
