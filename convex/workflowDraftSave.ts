...
import { getWorkflowForAgent, getWorkflowGraph, listWorkflowEdges, listWorkflowNodes } from './workflowCore';
import { deleteOrQueueWorkflowNodeMedia } from './workflowMediaDeletion';
import { workflowLayoutOrientationValidator, workflowNodeKindValidator } from './workflowValidators';
...
const draftEdgeValidator = v.object({
...
export const save = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (workflow.updatedAt !== args.baselineUpdatedAt) {
      throw new Error('This workflow changed elsewhere. Reset to load the latest version before saving.');
    }
    ...
    const existingNodeById = new Map(existingNodes.map((node) => [node._id, node]));
    const retainedNodeIds = new Set<Id<'workflowNodes'>>();
    const serviceIds = new Set(args.nodes.flatMap((node) => node.allowedAppointmentServiceIds ?? []));
    for (const serviceId of serviceIds) {
      const service = await ctx.db.get(serviceId);
      if (!service || service.agentId !== agent._id) throw new Error('Appointment service not found');
    }
    for (const node of args.nodes) {
      if (!node.persistedNodeId) continue;
      const existingNode = existingNodeById.get(node.persistedNodeId);
      if (!existingNode) throw new Error('Workflow node not found');
      if (existingNode.kind !== node.kind) throw new Error('Workflow node kind cannot be changed');
    ...
    for (const node of args.nodes) {
      const values = {
        ...
        allowedAppointmentServiceIds: node.kind === 'bookAppointment'
    ...
    if (!savedWorkflow) throw new Error('Workflow not found');
