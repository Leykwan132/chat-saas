...
test("AI reply worker always runs the planner and injects language into generated replies", () => {
  ...
  expect(inboxSource).toContain("configuredAgent.generateObject(");
  expect(inboxSource).toContain("schema: aiReplyOutputSchema");
  expect(inboxSource).toContain("result.object.messages");
...
test("AI reply worker sends matched Send message text exactly", () => {
  ...
  expect(inboxSource).toContain("responseMessages = plannedMessage ? [plannedMessage] : [];");
});
