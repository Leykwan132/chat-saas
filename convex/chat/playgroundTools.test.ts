...
test("playground exposes only context and availability tools while testing", () => {
  expect(threadsSource).toContain("playgroundAvailabilityOnly");
  expect(threadsSource).toContain('toolName !== "fetchContext" && toolName !== "checkAvailability"');
